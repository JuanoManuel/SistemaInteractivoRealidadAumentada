	...
	public class LeanDragTranslate : MonoBehaviour
	{
		...
		private void Translate(Vector2 screenDelta)
		{
			...
			if (camera != null)
			{
				...
				// Convert back to world space Original: transform.position = camera.ScreenToWorldPoint(screenPoint);
				Vector3 tempPos = camera.ScreenToWorldPoint(screenPoint);
				tempPos.y = transform.position.y;
				//transform.position = new Vector3(camera.ScreenToWorldPoint(screenPoint).x, transform.position.y, camera.ScreenToWorldPoint(screenPoint).z);
